...
void dumpGlobalConfig() {
    ...
    LOG(INFO) << "mtu_length: " << config.mtu_length;
    LOG(INFO) << "handshake_port: " << config.handshake_port;
    LOG(INFO) << "workers_per_ctx: " << config.workers_per_ctx;
    LOG(INFO) << "verbose: " << (config.verbose ? "true" : "false");
    LOG(INFO) << "slice_size: " << config.slice_size;
    LOG(INFO) << "retry_cnt: " << config.retry_cnt;
    LOG(INFO) << "==========================";
}
...
void updateGlobalConfig(ibv_device_attr &device_attr) {
    GlobalConfig &config = globalConfig();
    if (device_attr.max_cqe < config.max_cqe) {
        LOG(WARNING) << "Update max_cqe from " << config.max_cqe << " to "
                     << device_attr.max_cqe;
        config.max_cqe = device_attr.max_cqe;
    }
    if (device_attr.max_qp_wr < config.max_wr) {
        LOG(WARNING) << "Update max_wr from " << config.max_wr << " to "
                     << device_attr.max_qp_wr;
        config.max_wr = device_attr.max_qp_wr;
    }
    if (device_attr.max_sge < config.max_sge) {
        LOG(WARNING) << "Update max_sge from " << config.max_sge << " to "
                     << device_attr.max_sge;
